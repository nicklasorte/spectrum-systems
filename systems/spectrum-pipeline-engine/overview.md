...
- **Orchestration Responsibilities**: validate contract versions, enforce deterministic sequencing (minutes → agenda → readiness), align provenance across upstream artifacts, publish replay-ready manifests, and block partial emission on failure boundaries.
...
- **Local Prohibitions**: must not redefine schemas, mutate upstream artifacts outside governed transforms, or emit readiness scoring/rules that diverge from spectrum-systems contracts and standards manifest pins.
