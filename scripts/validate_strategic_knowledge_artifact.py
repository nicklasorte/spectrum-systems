...
def main() -> int:
    ...
    parser.add_argument(
        "--emit-trace",
        action="store_true",
        help="Emit trace span payload to stderr in addition to governed decision JSON output.",
    )
    ...
    if args.emit_trace:
        print(json.dumps({"trace_id": decision["trace_id"], "trace_spans": decision["trace_spans"]}, indent=2), file=sys.stderr)
