...
def main()->int:
 ...
 norm=normalize_failure(obs);cand=build_candidate(norm);auth=authorize(cand);exe=execute_repair(cand,auth,a.apply_repair);summ=summarize(obs,norm,cand,auth,exe)
 for n,o in [('pr_ci_failure_observation_record.json',obs),('pr_failure_normalization_packet.json',norm),('pr_repair_candidate_record.json',cand),('pr_repair_authorization_record.json',auth),('pr_repair_execution_record.json',exe),('pr_repair_attempt_summary_record.json',summ)]:
