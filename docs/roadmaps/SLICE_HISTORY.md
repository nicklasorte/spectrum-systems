...
- **2026-04-07 — BATCH-GOV-FIX-03**: Repaired external-path governance preflight handling by making checker file classification path-safe and defaulting external files to fail-closed text validation; updated wrapper path display handling and aligned run-prompt tests to canonical BLOCKED output.

- **2026-04-07 — BATCH-GOV-FIX-04**: Added direct checker-level external-path regression tests for `evaluate_prompt_file()` to lock fail-closed valid/invalid behavior and missing-reference diagnostics independent of wrapper path normalization.
