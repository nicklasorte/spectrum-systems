...
- Produces readiness and restoration **signals** (not downstream owner inputs), and emits control advisories for downstream canonical owners.
...
HOP does **not** own release, restoration, advancement, validation-evidence, control, guard, review, or judgment authority.

- It does not gate advancement/release outcomes; REL/GOV/CDE remain canonical owners.
- It does not perform fail-closed guard actions; SEL/ENF remain canonical owners.
- It does not make final policy/judgment inputs; JDX/CDE/GOV remain canonical owners by cluster.
...
2. **Eval hardness governance pending**: baseline-ceiling guard outcome checks for extraction (`score < 0.85` search) are design-specified but not yet executable until HOP-006B lands.
...
- Registry phrasing in non-owning sections must continue to use boundary clarification language (`external to`, `never emits owner input`, `downstream canonical owners`) rather than ownership claims.
...
1. Reintroduction of authority-shaped identifiers in HOP docs/code (e.g., `advancement_signal` should be used instead of owner-shaped identifiers).
2. Non-owning registry section language drifting into ownership claims (e.g., “HOP emits advancement owner input”).
