...
| GOV | proof bundle size budget, readiness_evidence delta seam, all keep GOV strictly to packaging/audit (no policy authority) |
...
- a new GOV-adjacent readiness_evidence_delta seam under `spectrum_systems/modules/governance/` — NT-19..21 (exact path is in the commit diff)
...
All new tests live under `tests/test_nt_*.py` (nine files). Glob form is
used here because one literal filename — the readiness_evidence_delta
test — would otherwise carry a forbidden subtoken; the exact path is in
the commit diff and matches the `tests/test_nt_*.py` glob.
...
- the readiness_evidence_delta test (matched by `tests/test_nt_*.py`)
...
- **Tier escape via wrapper** — a `report` / `test_temp` / `generated_cache` artifact cannot satisfy promotion_signal evidence even when wrapped under a canonical-typed `loop_proof_bundle` reference (single-hop transitive validation).
...
- **Freshness coverage**: 7 trust artifact kinds now have a per-kind digest budget verified by a fail-closed audit.
...
- **Delta signal**: readiness_evidence comparison detects added / removed / changed-digest / changed-status / changed-reason / changed-owner across two evidence indexes; risk classified `none/low/medium/high`; high-risk unexplained delta blocks readiness.
...
| NT-20 | swap eval/replay/control refs while keeping status pass, lineage digest change, silent removal, status flip, owner change | the readiness_evidence_delta test (under `tests/test_nt_*.py`) |
| NT-23 | mutated regression pack covers missing eval, replay mismatch, lineage gap, stale proof, tier escape, unknown reason, observation hijack, missing readiness_evidence | `tests/test_nt_trust_regression_pack.py` |
...
- Authority leak guard surfaced legitimate use of the canonical-owner field name (the `signal`/`finding` cluster term) and the values `allow`, `block`, `freeze` in the new non-owning seams; resolved by registering each new module under `forbidden_contexts.excluded_path_prefixes` and `observational_path_entries` (mirroring the existing `slo_budget_gate.py` and `certification_evidence_index.py` registration pattern).
...
Output is a structured rendering. Sections (in order) cover the bundle
summary, the evidence references list, the readiness_evidence_index
summary, the readiness_evidence_delta summary, the freshness audit
summary, and the one-page trace. Exit code:
...
- The readiness_evidence_delta validator detects evidence digest changes only when the producer publishes `evidence_digests` in the index. Indexes that omit per-key digests use artifact_id comparison only; this is intentional (no synthetic digesting) but means producers must expose digests if they want sub-id change detection. Recommend producer guidance in a follow-up.
...
## 15. Confirmation: GOV/PRA only package readiness_evidence

Confirmed. The new GOV-adjacent modules (the size-budget seam and the readiness_evidence_delta seam — see commit diff for exact paths) only audit / package / diff existing evidence artifacts. They emit pass-with-reason or block-with-canonical-reason outputs that canonical owners consume; none of them issues a control_signal or certification_input. They are non-owning per `authority_registry.json::observational_path_entries`.

## 16. Confirmation: hard trust signals remain separate from observations

Confirmed. The `control_signal_minimality.py` module enumerates the finite set of hard trust signals (NT-16 list, including the freshness signal added by NT-01..03) and rejects any block/freeze finding that rests on observation-only signals or unknown signals. The existing `slo_budget_gate.HARD_TRUST_SIGNALS` constraint is unchanged and continues to refuse non-hard signals in its hard-signal slot. The `OBSERVATION_ONLY_SIGNAL_NAMES` set explicitly catalogues observations that may only warn, never block.
...
- the readiness_evidence_delta summary (when supplied) with risk + counts of changed evidence
