...
| [ROW: PQX-PROT-11] | Protocol execution envelope | Protocol-layer packaging for PQX execution evidence across tracks, including queue-native audit bundles and future non-queue artifacts | Provides one protocol-level wrapper for cross-track evidence transport without duplicating queue-native audit packaging | SOURCE GAP | artifact system, queue audit bundle outputs | ADD NEW | `pqx_execution_envelope.schema.json` | protocol execution envelope artifact | queue + protocol governance + audit | L | QUEUE-11 | Protocol envelope wraps required evidence references and preserves queue-native audit bundles as source artifacts | governance | PLANNED |
