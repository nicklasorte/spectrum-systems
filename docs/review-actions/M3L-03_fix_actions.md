...
| MF-09 builder secretly emits readiness or gate decisions | resolved | schema `additionalProperties: false` forbids any field outside the declared list; declared list contains no `*_ready_*`, no `gate_status`, no `promote`, no `approve`, no `enforce` | `test_record_has_no_readiness_or_gate_fields` | `python -m pytest tests/test_gate_shard_parity_record.py -q` |
