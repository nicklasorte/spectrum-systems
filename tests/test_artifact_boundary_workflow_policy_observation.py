...
def test_contract_preflight_ref_signal_pull_request_uses_pr_sha_pair() -> None:
    text = _workflow_text()
    ...
    assert 'echo "head_ref=${{ github.event.pull_request.head.sha }}" >> "$GITHUB_OUTPUT"' in text
