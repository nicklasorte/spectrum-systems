...
def test_non_owner_using_promotion_decision_fails(tmp_path: Path, vocab) -> None:
    """A non-owner system named in an action-verb construct must fail closed.

    Under the narrowed AUTH-SHAPE-01F model, the violation requires a
    ``<3-letter system> <action-verb>`` pattern. Generic governance nouns on
    their own no longer trigger a violation.
    """
    ...
    _write(
        repo,
        rel,
        "# HOP promotes readiness signals for downstream consumers.\n"
        "PROMOTION_SIGNAL = {'value': 'observation_only'}\n",
    )
...
def test_non_owner_using_rollback_record_fails(tmp_path: Path, vocab) -> None:
    """A non-owner naming itself with a rollback action verb must fail closed."""
    repo = _seed_repo(tmp_path)
    rel = "spectrum_systems/modules/hop/rollback_observer.py"
    _write(
        repo,
        rel,
        "# HOP rolls back state when a downstream signal arrives.\n"
        "rollback_signal = {'steps': []}\n",
    )
    result = evaluate_preflight(
        repo_root=repo, changed_files=[rel], vocab=vocab, mode="suggest-only"
    )
...
def test_violation_payload_carries_required_fields(tmp_path: Path, vocab) -> None:
    """Diagnostic payload must carry the canonical fields when an action-verb leak is detected."""
    repo = _seed_repo(tmp_path)
    rel = "spectrum_systems/modules/hop/cert_observer.py"
    _write(
        repo,
        rel,
        "# HOP certifies the result here without owning certification.\n"
        "x = 'leak'\n",
    )
    result = evaluate_preflight(
        repo_root=repo, changed_files=[rel], vocab=vocab, mode="suggest-only"
