...
def test_missing_authority_shape_output_blocks():
 d=_base(); d['gate_status']='block'; d['failure_classes']=['missing_required_artifact']; assert d['gate_status']=='block'
...
def test_observation_only_required(tmp_path: Path):
 ...
 out=evaluate_agl_repo_mutating_evidence(repo_mutating=True, clp_result_path=str(p)); assert out['compliance_status']=='BLOCK'

def test_agl_missing_clp_blocks(tmp_path: Path):
 out=evaluate_agl_repo_mutating_evidence(repo_mutating=True, clp_result_path=str(tmp_path/'none.json')); assert out['compliance_status']=='BLOCK'
