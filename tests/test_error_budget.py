...
def test_legacy_error_budget_tracker_surface_is_available() -> None:
    tracker = ErrorBudgetTracker(window_size=2)
    update_error_budget(
        "run-1",
        "healthy",
        {
    ...
    burn = compute_burn_rate(tracker=tracker)
    assert burn["overall"] == 0.0
    assert tracker.run_count() == 1


def test_import_error_budget_tracker_symbol_exists() -> None:
    module = importlib.import_module("spectrum_systems.modules.runtime.error_budget")
    assert hasattr(module, "ErrorBudgetTracker")


def test_legacy_tracker_fails_closed_on_bad_sli_value() -> None:
    tracker = ErrorBudgetTracker()
    with pytest.raises(ErrorBudgetError, match="must be between 0.0 and 1.0"):
        update_error_budget(
            "run-1",
            "healthy",
            {
