...
def test_cli_returns_nonzero_on_violation(monkeypatch, tmp_path: Path) -> None:
    ...
    # Narrowed AUTH-SHAPE-01F detection: violation requires a 3-letter system
    # token followed by an action verb. ``HOP promotes`` triggers the
    # promotion cluster.
    sandbox_path.write_text(
        "# HOP promotes readiness here.\npromotion_signal = 'pending'\n",
        encoding="utf-8",
    )
...
def test_cli_runs_without_jsonschema(tmp_path: Path) -> None:
    ...
    # Narrowed AUTH-SHAPE-01F detection: violation requires a 3-letter system
    # token followed by an action verb (``HOP promotes`` triggers the promotion
    # cluster). The safe file uses presence-of-evidence framing (no verb) and
    # must not be flagged.
