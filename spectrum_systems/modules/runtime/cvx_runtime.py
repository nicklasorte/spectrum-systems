"""CVX deterministic runtime for ADV-001."""
...
class CVXRuntimeError(ValueError):
    """Raised when CVX runtime checks fail closed."""
...
def _require_mapping(value: Any, field: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise CVXRuntimeError(f"{field} must be an object")
    return value
...
def build_effectiveness_artifact(*, eval_artifact: Mapping[str, Any], replay_result: Mapping[str, Any]) -> dict[str, Any]:
    artifact = {
        "artifact_type": "cvx_instability_report",
        "schema_version": "1.0.0",
        "artifact_id": _stable_id("cvx-effect", [eval_artifact, replay_result]),
    ...
    validate_artifact(artifact, "cvx_instability_report")
    return artifact
...
def run_red_team_round(*, fixture: Mapping[str, Any]) -> dict[str, Any]:
    ...
    artifact = {
        ...
        "schema_version": "1.0.0",
        "artifact_id": _stable_id("cvx-bundle", base),
    ...
    validate_artifact(artifact, "cvx_consistency_bundle")
    return artifact
