"""DEX deterministic runtime for ADV-001."""
...
class DEXRuntimeError(ValueError):
    """Raised when DEX runtime checks fail closed."""
...
def _require_mapping(value: Any, field: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise DEXRuntimeError(f"{field} must be an object")
    return value
...
def build_effectiveness_artifact(*, eval_artifact: Mapping[str, Any], replay_result: Mapping[str, Any]) -> dict[str, Any]:
    artifact = {
        "artifact_type": "dex_explanation_summary",
        "schema_version": "1.0.0",
        "artifact_id": _stable_id("dex-effect", [eval_artifact, replay_result]),
    ...
    validate_artifact(artifact, "dex_explanation_summary")
    return artifact
...
def run_red_team_round(*, fixture: Mapping[str, Any]) -> dict[str, Any]:
    ...
    artifact = {
        "artifact_type": "dex_explanation_bundle",
        "schema_version": "1.0.0",
        "artifact_id": _stable_id("dex-bundle", base),
    ...
    validate_artifact(artifact, "dex_explanation_bundle")
    return artifact
