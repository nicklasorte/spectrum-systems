...
from spectrum_systems.modules.runtime.trace_engine import create_trace_store, start_trace, validate_trace_context
...
def _case_orphaned_lineage_parent_refs(trace_store: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    isolated_store = trace_store or create_trace_store()
    trace_id = start_trace(
        {"trace_id": "trace-chaos-orphan", "run_id": "run-chaos-orphan"},
        store=isolated_store,
    )
    errors = validate_trace_context(
        trace_id,
        span_id="span-missing-parent",
        store=isolated_store,
    )
...
def run_governed_failure_injection(
    *,
    ...
    for case in selected_cases:
        if case.case_id == "trace_orphaned_parent_refs":
            outcome = _case_orphaned_lineage_parent_refs(trace_store=trace_store)
        else:
            outcome = case.fn()
        case_identity = {
            "run_id": run_id,
