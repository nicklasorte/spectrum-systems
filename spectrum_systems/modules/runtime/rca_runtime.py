"""RCA deterministic runtime for MASTER-ROADMAP-001."""
...
class RCARuntimeError(ValueError):
    """Raised when RCA runtime checks fail closed."""
...
def _require_mapping(value: Any, field: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise RCARuntimeError(f"{field} must be an object")
    return value
...
def build_effectiveness_artifact(*, eval_artifact: Mapping[str, Any], replay_result: Mapping[str, Any]) -> dict[str, Any]:
    artifact = {
        "artifact_type": "rca_attribution_bundle",
        "schema_version": "1.0.0",
        "artifact_id": _stable_id("rca-effect", [eval_artifact, replay_result]),
    ...
    validate_artifact(artifact, "rca_attribution_bundle")
    return artifact
...
def run_red_team_round(*, fixture: Mapping[str, Any]) -> dict[str, Any]:
    ...
    artifact = {
        "artifact_type": "rca_attribution_bundle",
        "schema_version": "1.0.0",
        "artifact_id": _stable_id("rca-bundle", base),
