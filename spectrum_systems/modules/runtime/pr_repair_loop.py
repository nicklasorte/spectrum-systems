...
MAX_ATTEMPTS=2
KNOWN_CLASSES={"authority_shape_violation","authority_leak_observation","pytest_selection_missing","schema_validation_failure","contract_compliance_observation","missing_shard_artifact","invalid_shard_artifact","pr_gate_observation","dashboard_test_failure","unknown_failure"}
AUTO_REPAIR_CLASSES={"authority_shape_violation","authority_leak_observation","pytest_selection_missing","schema_validation_failure"}
...
def observe_failures(pr_number:int, repo:str, head_sha:str, attempt_number:int, input_dir:Path)->dict[str,Any]:
    ...
    for p in paths:
      ...
      if d is not None:
        ...
        if status in {'fail','failed','not_ready_for_control_review'}:
...
def normalize_failure(obs:dict[str,Any])->dict[str,Any]:
    ...
    mapping=[('authority_shape','authority_shape_violation'),('authority_leak','authority_leak_observation'),('pr_gate','pr_gate_observation'),('contract_preflight','contract_compliance_observation')]
    for k,v in mapping:
...
def prepare_repair_readiness(candidate:dict[str,Any])->dict[str,Any]:
...
def execute_repair(candidate:dict[str,Any], auth:dict[str,Any], apply_repair:bool=False)->dict[str,Any]:
    status='not_ready_for_control_review'; changed=[];cmd=[];tests=[]
    if auth['repair_readiness_status']=='ready_for_control_review' and apply_repair:
    ...
    'source_artifacts_used':candidate['source_artifacts_used'],'trace_refs':candidate['trace_refs'],'reason_codes':candidate['reason_codes'],'authority_scope':'observation_only',
    'files_changed':changed,'commands_run':cmd,'tests_run':tests,'pqx_execution_ref':'PQX:execution','evl_result_refs':['EVL:evaluation'],'downstream_review_refs':['TPA:scope_check','CDE:control_review_input','SEL:policy_observation_input'],'tests_weakened':False,'skipped_required_gate':False,'status':status}
