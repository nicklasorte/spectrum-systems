"""SIMX deterministic runtime for MASTER-ROADMAP-001."""
...
class SIMXRuntimeError(ValueError):
    """Raised when SIMX runtime checks fail closed."""
...
def _require_mapping(value: Any, field: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise SIMXRuntimeError(f"{field} must be an object")
    return value
...
def build_effectiveness_artifact(*, eval_artifact: Mapping[str, Any], replay_result: Mapping[str, Any]) -> dict[str, Any]:
    artifact = {
        "artifact_type": "simx_drift_report",
        "schema_version": "1.0.0",
        "artifact_id": _stable_id("simx-effect", [eval_artifact, replay_result]),
    ...
    validate_artifact(artifact, "simx_drift_report")
    return artifact
...
def run_red_team_round(*, fixture: Mapping[str, Any]) -> dict[str, Any]:
    ...
    artifact = {
        "artifact_type": "simx_simulation_integrity_bundle",
        "schema_version": "1.0.0",
        "artifact_id": _stable_id("simx-bundle", base),
    ...
    validate_artifact(artifact, "simx_simulation_integrity_bundle")
    return artifact
