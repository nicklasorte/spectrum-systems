"""HIX deterministic runtime for ADV-001."""
...
class HIXRuntimeError(ValueError):
    """Raised when HIX runtime checks fail closed."""
...
def _require_mapping(value: Any, field: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise HIXRuntimeError(f"{field} must be an object")
    return value
...
def build_effectiveness_artifact(*, eval_artifact: Mapping[str, Any], replay_result: Mapping[str, Any]) -> dict[str, Any]:
    artifact = {
        "artifact_type": "hix_feedback_exchange_record",
        "schema_version": "1.0.0",
        "artifact_id": _stable_id("hix-effect", [eval_artifact, replay_result]),
    ...
    validate_artifact(artifact, "hix_feedback_exchange_record")
    return artifact
...
def run_red_team_round(*, fixture: Mapping[str, Any]) -> dict[str, Any]:
    ...
    artifact = {
        "artifact_type": "hix_human_interaction_bundle",
        "schema_version": "1.0.0",
        "artifact_id": _stable_id("hix-bundle", base),
    ...
    validate_artifact(artifact, "hix_human_interaction_bundle")
    return artifact
