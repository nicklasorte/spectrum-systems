...
def _source_refs(inputs: NextStepInputs) -> list[dict[str, Any]]:
    return [
        ...
        for row in inputs.source_refs
    ]


def _is_complete(payload: Any, *, positive_statuses: set[str] | None = None) -> bool:
    if not isinstance(payload, dict):
        return False
    if positive_statuses is None:
        positive_statuses = {"pass", "implemented", "complete", "ready", "success"}
    ...
    if isinstance(readiness, dict) and readiness.get("ready") is True:
        return True
    decision = str(payload.get("review_decision", "")).lower()
    return decision in {"pass", "approve", "approved"}
...
def _derive_done(inputs: NextStepInputs) -> set[str]:
    ...
    h01_artifacts_present = any(
        ref.present and ref.path.startswith("contracts/review_artifact/H01")
        for ref in inputs.source_refs
    ) and any(
        ref.present and ref.path.startswith("docs/reviews/H01")
        for ref in inputs.source_refs
    )
    if h01_artifacts_present and {"BLF-01", "RFX-04", "RMP-SUPER-01"}.issubset(done):
        done.add("H01")
...
def build_next_step_report(repo_root: Path) -> tuple[dict[str, Any], bool]:
    ...
    if missing_required:
        payload = blocked_payload(
            reason_codes=[f"missing_required_artifact:{path}" for path in missing_required],
            source_refs=source_refs,
            warnings=["Next-step recommendation blocked due to missing required artifact inputs."],
        )
        return payload, True
    ...
    if isinstance(advisory, dict):
        for row in advisory.get("top_5", []):
            if isinstance(row, dict) and isinstance(row.get("system_id"), str):
                advisory_top.append(row["system_id"])
    ...
    for idx, item in enumerate(LOCKED_SEQUENCE, start=1):
        ...
        ranked.append(
            {
                ...
                "id": item.id,
                "work_item": item.work_item,
                "blocked_by": deps_missing,
                "eligible_now": not deps_missing and status != "complete",
            }
        )
        if deps_missing and status != "complete":
            rejected.append(
                {
                    "work_item": item.id,
                    "reason": f"{item.id} must wait for dependencies: {', '.join(deps_missing)}.",
                }
            )
    ...
    if selected is not None:
        selected_next_step = {
            ...
            "execution_prompt_hint": (
                "Build Fix Integrity Proof and proof-bound closure gate."
                if selected.id == "RFX-PROOF-01"
                else f"Execute {selected.id} with strict dependency closure evidence."
            ),
        }
    else:
    ...
    red_team_findings = run_redteam(
        done,
        selected.id if selected else None,
        missing_required,
        advisory_top,
        h01_claimed=h01_claimed,
    )

    status = "blocked" if (selected_next_step is None or reason_codes) else "pass"
    readiness_state = "blocked" if status == "blocked" else "ready"
...
def write_next_step_report(repo_root: Path, out_path: Path | None = None) -> tuple[dict[str, Any], bool]:
    ...
    target = out_path or (repo_root / "artifacts" / "next_step_decision_report.json")
