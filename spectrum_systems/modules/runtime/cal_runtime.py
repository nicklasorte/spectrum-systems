"""CAL deterministic runtime for MASTER-ROADMAP-001."""
...
class CALRuntimeError(ValueError):
    """Raised when CAL runtime checks fail closed."""
...
def _require_mapping(value: Any, field: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise CALRuntimeError(f"{field} must be an object")
    return value
...
def build_effectiveness_artifact(*, eval_artifact: Mapping[str, Any], replay_result: Mapping[str, Any]) -> dict[str, Any]:
    artifact = {
        "artifact_type": "cal_confidence_budget_status",
        "schema_version": "1.0.0",
        "artifact_id": _stable_id("cal-effect", [eval_artifact, replay_result]),
    ...
    validate_artifact(artifact, "cal_confidence_budget_status")
    return artifact
...
def run_red_team_round(*, fixture: Mapping[str, Any]) -> dict[str, Any]:
    ...
    artifact = {
        "artifact_type": "cal_calibration_bundle",
        "schema_version": "1.0.0",
        "artifact_id": _stable_id("cal-bundle", base),
    ...
    validate_artifact(artifact, "cal_calibration_bundle")
    return artifact
