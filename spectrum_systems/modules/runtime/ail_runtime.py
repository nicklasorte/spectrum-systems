"""AIL deterministic runtime for MASTER-ROADMAP-001."""
...
class AILRuntimeError(ValueError):
    """Raised when AIL runtime checks fail closed."""
...
def _require_mapping(value: Any, field: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise AILRuntimeError(f"{field} must be an object")
    return value
...
def build_effectiveness_artifact(*, eval_artifact: Mapping[str, Any], replay_result: Mapping[str, Any]) -> dict[str, Any]:
    artifact = {
        ...
        "schema_version": "1.0.0",
        "artifact_id": _stable_id("ail-effect", [eval_artifact, replay_result]),
    ...
    validate_artifact(artifact, "ail_trend_cluster_report")
    return artifact
...
def run_red_team_round(*, fixture: Mapping[str, Any]) -> dict[str, Any]:
    ...
    artifact = {
        "artifact_type": "ail_intelligence_bundle",
        "schema_version": "1.0.0",
        "artifact_id": _stable_id("ail-bundle", base),
    ...
    validate_artifact(artifact, "ail_intelligence_bundle")
    return artifact
