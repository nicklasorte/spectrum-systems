"""DEP deterministic runtime for MASTER-ROADMAP-001."""
...
class DEPRuntimeError(ValueError):
    """Raised when DEP runtime checks fail closed."""
...
def _require_mapping(value: Any, field: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise DEPRuntimeError(f"{field} must be an object")
    return value
...
def build_effectiveness_artifact(*, eval_artifact: Mapping[str, Any], replay_result: Mapping[str, Any]) -> dict[str, Any]:
    artifact = {
        ...
        "schema_version": "1.0.0",
        "artifact_id": _stable_id("dep-effect", [eval_artifact, replay_result]),
    ...
    validate_artifact(artifact, "dep_regression_surface_report")
    return artifact
...
def run_red_team_round(*, fixture: Mapping[str, Any]) -> dict[str, Any]:
    ...
    artifact = {
        ...
        "schema_version": "1.0.0",
        "artifact_id": _stable_id("dep-bundle", base),
    ...
    validate_artifact(artifact, "dep_dependency_bundle")
    return artifact
