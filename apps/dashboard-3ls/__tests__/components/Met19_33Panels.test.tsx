...
describe('MET-19-33 dashboard sections', () => {
  ...
  it('keeps MET-19-33 heavy panels out of Overview', async () => {
    setupFetch({
      candidate_closure: {
  ...
  it('overview remains compact when MET-19-33 blocks are missing', async () => {
    setupFetch({});
    render(<DashboardPage />);
    await waitFor(() => expect(screen.getByTestId('overview-tab')).toBeInTheDocument());
    expect(screen.queryByTestId('candidate-closure-section')).not.toBeInTheDocument();
  ...
  it('compact overview does not render MET-19-33 item lists', async () => {
    setupFetch({
      candidate_closure: {
    ...
    render(<DashboardPage />);
    await waitFor(() => expect(screen.getByTestId('overview-tab')).toBeInTheDocument());
    expect(screen.queryByTestId('candidate-closure-items')).not.toBeInTheDocument();
    expect(screen.queryByTestId('debug-explanation-entries')).not.toBeInTheDocument();
    expect(screen.queryByTestId('evl-handoff-items')).not.toBeInTheDocument();
    expect(screen.queryByTestId('artifact-integrity-classified-paths')).not.toBeInTheDocument();
  });
