...
const EDGE_GROUPS: Array<{ key: string; label: string; stroke: string; dash?: string }> = [
  { key: 'core', label: 'Core flow (always visible)', stroke: '#1d4ed8' },
  { key: 'failure', label: 'Failure path (Failure Path mode)', stroke: '#ea580c' },
  { key: 'selected', label: 'Selected node neighborhood', stroke: '#334155', dash: '3 3' },
  { key: 'registry', label: 'Dense registry diagnostics (Full Registry)', stroke: '#9ca3af', dash: '5 4' },
];

export function GraphLegend() {
  return (
    <div className="border border-slate-200 dark:border-slate-700 rounded p-3 text-xs space-y-3 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100" data-testid="graph-legend">
